...
class fcos_Enhanced(AnchorFreeHead):
    ...
    def _init_layers(self):
        ...
        self.cls_convs.append(
            LDCA(self.feat_channels, 3, dim_reduction=4, mlp_ratio=3)
        )
        ...
        self.num_anchors = 1
        self.conv_cls = BACA(self.feat_channels, ks=self.dcim_ks, num_class=self.cls_out_channels, num_anchor=self.num_anchors)
    ...
    def init_weights(self):
        ...
        torch.nn.init.constant_(self.conv_cls.cls_conv.bias, bias_cls)
        nn.init.constant_(self.conv_cls.conv_offset[-1].weight, 0)
        nn.init.constant_(self.conv_cls.conv_offset[-1].bias, 0)
    ...
    def forward(self, feats, img_metas=None):
        ...
        mlvl_priors = self.prior_generator.grid_priors(featmap_sizes, dtype=feats[0].dtype, device=feats[0].device)

        img_metas_list = [img_metas for i in range(len(self.strides))]
        return multi_apply(self.forward_single, feats, self.scales, self.strides, mlvl_priors, img_metas_list)

    def forward_single(self, x, scale, stride, priors, img_meta):
        ...
        bbox_pred = self.conv_reg(reg_feat)  # (B, 4, H, W)

        B, C, H, W = x.shape
        # predict_bbox = scale(bbox_pred).float().exp()
        predict_bbox = scale(bbox_pred).float().clamp(min=0) * stride
        bbox_pred_grad_mul = (1 - self.gradient_mul) * predict_bbox.detach() + self.gradient_mul * predict_bbox
        bbox_pred_grad_mul = bbox_pred_grad_mul.permute(0, 2, 3, 1).reshape(-1, 4)
        if not self.training and (img_meta is None):
            img_meta = [dict(img_shape=(1280, 800))]

        priors = priors.repeat(B, 1)
        bbox_pred_decode = self.bbox_coder.decode(priors, bbox_pred_grad_mul, max_shape=img_meta[0]['img_shape'])
        bbox_pred_decode = bbox_pred_decode.reshape(B, H, W, self.num_anchors, 4)  # 原图上的绝对坐标
        decode_offsets = self.decode_bbox_offset(bbox_pred_decode) / stride # 原图上的绝对坐标
        decode_offsets = decode_offsets.reshape(B, -1, H, W)
        cls_score = self.conv_cls(cls_feat, decode_offsets)
